// app.js — Nexus Salon (Firebase compat, sin imports, PIN funcionando)
...
function saveState() {
  const clone = { ...state };
  delete clone.user; // no guardamos el objeto user completo
...
// ========== EXPORT PDF (SEGURO, NO ROMPE APP) ==========
function exportTicketsToPDF() {
  const jsPDFLib = window.jspdf && window.jspdf.jsPDF;
  if (!jsPDFLib) {
    alert(
      "La librería de PDF (jsPDF) no se cargó. Verifica el script de jsPDF o tu conexión."
    );
    return;
  }
...
logoutBtn.addEventListener("click", () => {
  // salir a PIN sin cerrar sesión de Google
  showPinScreen();
});
