// app.js — Nexus Salon (versión final)
...
// ticket que se está editando, null si es nuevo
...
/* ========== BRANDING (Firestore sin Storage) ========== */
...
ticketsTableBody.addEventListener("click", async (e) => {
  ...
  if (action === "edit") {
    ...
    quantityInput.value = ticket.quantity || 1;
    unitPriceInput.value = ticket.unitPrice || 0;
    tipAmountInput.value = ticket.tipAmount || 0;
    recalcTotal();

    // técnica
    if (ticket.technician === "Cynthia" ||
        ticket.technician === "Carmen" ||
        ticket.technician === "Yerika") {
    ...
    formMessage.textContent = `Editando ticket #${ticket.number}. Al guardar se actualizará.`;
